...
make the known verification command the first shell command, do not run build,
lint, broad file scans such as `Glob **`, `ls`, `find`, `tree`, or `rg --files`,
or project-document rediscovery before it, and run build/lint only after tests
pass unless the command fails or the user explicitly asks for
